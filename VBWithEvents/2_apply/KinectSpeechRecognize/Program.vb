...
Module Program
    Friend WithEvents SpeechEvents As SpeechRecognitionEngine
    ...
    Sub Main(args As String())
        Try
            ...
            SpeechEvents = New SpeechRecognitionEngine(info.Id)
            SpeechEvents.LoadGrammar(_grammar)
            ...
            Dim audio As KinectAudioSource = sensor.AudioSource
            Using s As Stream = audio.Start
                ' 認識エンジンに音声ストリームを設定する
                SpeechEvents.SetInputToAudioStream(s, New SpeechAudioFormatInfo(
                                                EncodingFormat.Pcm,
                                                16000,
                                                16,
                                                1,
                ...
                ' 非同期で、音声認識を開始する
                SpeechEvents.RecognizeAsync(RecognizeMode.Multiple)
                Console.ReadLine()
                Console.WriteLine("Stopping recognizer ...")

                ' 音声認識を停止する
                SpeechEvents.RecognizeAsyncStop()
            End Using
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub engine_SpeechRecognized(sender As Object,
                                        e As SpeechRecognizedEventArgs) _
                                    Handles SpeechEvents.SpeechRecognized
