...
Class MainWindow
    ...
    Private Sub InitOpenCV()
        Me.OpenCVImage = New IplImage(Me.Kinect.ColorStream.FrameWidth,
                                      Me.Kinect.ColorStream.FrameHeight,
                                      BitDepth.U8,
                                      3)

        Me.OpenCVGrayImage = New IplImage(Me.Kinect.ColorStream.FrameWidth,
                                          Me.Kinect.ColorStream.FrameHeight,
                                          BitDepth.U8,
                                          1)
                    ...
                    If _skeletonFrame IsNot Nothing Then
                        ...
                        For Each _skeleton As Skeleton In skeletonData
                            ...
                            If head.TrackingState = JointTrackingState.Tracked Then
                                _point = Me.Kinect.MapSkeletonPointToColor(head.Position,
    ...
    Private Function ConvertToBitmap(ByVal cif As ColorImageFrame) As ImageSource
        ' 変数のインスタンス作成
        If OutputImage Is Nothing OrElse BGR32PixelData Is Nothing Then
    ...
    Private Function CheckFacePosition(headPosition As ColorImagePoint) As Rect
        ...
        Try
            ' 画像の切り取り
            Dim snipImage = SnipFaceImage(OpenCVGrayImage,
                                          headPosition,
                                          snipWidth,
                                          snipHeight)
            If snipImage IsNot Nothing Then
                Dim faces As CvSeq(Of CvAvgComp) = Cv.HaarDetectObjects(snipImage, Cascade, Storage)
                ' 顔を検出した場合
